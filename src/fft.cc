...
/* 
13 Aug 2018: changes by Schlather: all "static" declarations replaced
*/
 ...
 *  If *pmaxp == 1  There we more than 15 factors to ntot.  */
  ...
  /* schlather: returns 1=true on error else 0*/

  int j, jj, k, sqrtk, kchanged,
    m_fac, kt, maxf, maxp=0,
    *nfac = NFAC + 1;
...
Rboolean fft_work(double *a, double *b, int nseg, int n, int nspn,
		  int isn, double *work, int *iwork,
		  int maxf, int kt, int m_fac, int *NFAC)
{
  int nf, nspan, ntot,
    *nfac = NFAC + 1;
