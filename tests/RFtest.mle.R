...
########## define model + simulation
  ...
  res <- formatC(round(result$mle$param,dig=2),dig=2,width=5)
  res[!is.na(estparam)] <- "  . "
  cat("  mle:",res," [",format(result$mle.value,dig=2),
      "] (time:", format(ut[c(1)]),")\n")
  if (any(is.na( result$mle$param))) sfasdf
  est[is.na(estparam)] <-
    est[is.na(estparam)] + result$mle$param[is.na(estparam)]
