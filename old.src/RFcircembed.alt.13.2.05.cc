...
ce_param CIRCEMBED={false, false, TRIVIALSTARTEGY, -1e-7, 1e-3, 3, 
		    20000000.0, 0, 0, 0, 0};
...
{ 
  SetParamLoc(action, cutoff_a, intrinsic_r, &LOCAL_USER_PARAM, 
	      "LOCAL_USER_PARAM");
}
			...
			double inter_scaled_spacing)
{
  ...
  m[0]= 1 << (1 + (int) ceil(log(Rmax/inter_scaled_spacing) * INVLOG2 - EPSILON1000)); 
  for (i=1;i<dim;i++){
    m[i]=m[0];
  }
  totalm=pow(m[0], dim);
...
int circ_embed_get_initial_m(int *nn, int *m, int dim, ce_param* cepar)
{
  ...
  for (i=0;i<dim;i++){
    ...
    if (cepar->userfft) {
      ...
      m[i] =  NiceFFTNumber(factor * (unsigned long) nn[i]); 
    } else {
...
int circ_embed_with_initial_m(double *steps, bool anisotropy,
			     ...
			     long *twoRealmtot,
			     double **cc,
                             local_param_type localparam, // these two arguments are used in 
...
*/
  ...
  while (!positivedefinite && (trials<cepar->trials)){ 
    ...
    *twoRealmtot = 2 * sizeof(double) * mtot;
	...
	cur_crit |= (index[k]==halfm[k]);
      ...
      if (critical && cur_crit) {
        ...
        if (embed==Standard)
          ...
          c[dummy] = LocalCovFct(hx, dim, covnr, op, param, localparam, actcov, anisotropy, embed);
      }
    ...
    //for (i=0; i<(10<mtot ? 10:mtot); i++) printf("%d: %f; ", i, c[2*i]);
    //printf("\n");
	...
	if (GENERAL_PRINTLEVEL>=2) {
	  ...
	  if (GENERAL_PRINTLEVEL>=4) { /* just for printing the smallest 
	    ...
	    while (i<mtot-1) {
	      i++; twoi+=2;
	      if(c[twoi]<smallest) { smallest=c[twoi]; index=i; }
	    }
	    PRINTF("   The smallest eigenvalue is c[%d] =  %f \n",
		   index, smallest);
	  }
	}
      }
      ...
      if (!positivedefinite && (trials<cepar->trials)) { 
        assert( embed != Cutoff && embed != Intrinsic );
	// in these two embeddings we only do 1 trial
...
int init_circ_embed_local(key_type *key, int m)
{
  ...
  */
  ...
  { 
	...
	if (actcov>0) {
	  if ((multiply[actcov-1] = key->op[v-1])) { 
	    if (key->method[v-1] != CircEmbedLocal) {
  ...
  for (d=0; d<key->timespacedim; d++) {
    ...
    steps[d]=key->x[d][XSTEP];
    dummy = steps[d] * (Real) (key->length[d]-1);
  ...
  { 
    ...
    s->factor = 0.0;
    for (v=0; v<actcov; v++) {
      assert(fabs(s->param[v][SCALE] * s->param[v][INVSCALE]-1.0) < EPSILON);
      cov = &(CovList[covnr[v]]);
      s->param[v][DIAMETER] = diameter * s->param[v][INVSCALE];
      if ((cov->check!=NULL) && 
	  ((Xerror=cov->check(s->param[v], timespacedim, CircEmbedLocal)))!=0)
	goto ErrorHandling;
      // these steps are necessary, see the two cases in 3dBrownian!
      s->factor += s->param[v][INVSCALE] * s->param[v][INVSCALE] *
	// square.factor uses only KAPPA and DIAMETER
	2.0 * CovList[covnr[v]].square_factor(s->param[v]) *
	// 2.0 : see Stein (2002)
	s->param[v][VARIANCE];
    }
    s->factor = sqrt(s->factor);  // standard deviation of the Gaussian variables
    //                               in do_...

    v = 0;
    //   printf("** %f %f %f %f %f %f **\n", s->factor, s->param[v][INVSCALE],
//	  CovList[covnr[v]].square_factor(s->param[0]),
//	   s->param[v][VARIANCE], internalvariance, save);
  }

  if ((s->local=(double*) malloc(sizeof(Real) * s->totalpoints))==0) 
    {Xerror=ERRORMEMORYALLOCATION; goto ErrorHandling;} 
...
void do_circ_embed_local(key_type *key, bool add, int m, double *res )
{  
