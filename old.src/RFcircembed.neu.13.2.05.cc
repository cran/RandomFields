/*
 Authors 
 Yindeng, Jiang, *** please fill out, and do so for other files where you feel 
                    it is appropriate ***
  ...
  for (v=0; v<key->ncov; v++) {
    if ((key->method[v]==Method) && (key->left[v])) {
	 ...
	 do not remove the parenths around Method! */
      ...
      if (!CovList[covnr[*actcov]].implemented[Method])
    ...
    if (*actcov==0) { /* no covariance for the considered method found */
      if (key->traditional) return ERRORNOTINITIALIZED;
      return NOERROR_ENDOFLIST;
    }
  }
  return NOERROR;
}
...
int init_circ_embed(key_type * key, int m)
{
  ...
  if (FirstCheck_Cov(key, CircEmbed, param, false, covnr, multiply, &actcov) 
      != NOERROR)
    goto ErrorHandling;
  { 
    int timespacedim,v;
			 ...
			 int timespacedim)
{
  ...
  //printf("first: %f; second: %f; phi: %f; a0: %f; b: %f; a2: %f\n", 
  //firstderiv, secondderiv, phi, localparam[INTRINSIC_A0],
// localparam[INTRINSIC_B],localparam[INTRINSIC_A2]);
}
//end
...
int init_circ_embed_cutoff(key_type *key, int m)
{
  ...
  */
  if (FirstCheck_Cov(key, CircEmbedCutoff, param,  
...
int init_circ_embed_intrinsic(key_type *key, int m)
{
  ...
  /* break the for loop directly after first finding of a local circulant 
     ...
     extended programming would look like
  */
  if (FirstCheck_Cov(key, CircEmbedIntrinsic, param, 
