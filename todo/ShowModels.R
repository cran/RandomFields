...
#  stop("unchecked")
  ...
  if (is.null(model)) {
    ...
    stop("giving a model does not work yet")
    Model <- PrepareModel(model=model, param=param, method=method)
    trend <- Model$trend
    Model <- Model$model
    .Call("CheckModel", ..., PACKAGE="RandomFields")
    model.aniso <- any(names(unlist(Model))=="aniso")
    anisotropy <-
      anisotropy || model.aniso && (dim!=1) || is.matrix(empirical$e) 
    
    if (is.null(Mean)) Mean <- Model$trend
    err.mess <- "complicated models are not allowed yet"
    if (Model[[1]]=="+") {
      if (length(Model)>3) stop(err.mess)
      if (Model[[2]]=="nugget" || Model[[2]][length(Model[[2]])]=="nugget") {
        swap <- Model$model[[1]]
  ...
  #######################################################################
  namen <- GetModelNames(stationary=c("stationary", "variogram"),
                         isotropy="isotropic",
                         operator=FALSE, dim=dim)
  namen <- namen[namen != "0"]
  namen <- namen[namen != "constant"]  
  n <- length(namen)
  ...
  npar <- list()
  for (i in 1:n) {
    npar[[i]] <- .C("GetNrParameters",
                    as.integer(.C("GetModelNr", as.character(namen[i]), 
                                  nr = integer(1), PACKAGE="RandomFields")$nr),
                    k=integer(1),
                    PACKAGE="RandomFields")$k
  }
  npar <- lapply(npar, function(x) x <- rep(1, x - min(0, x) * 999))
  ...
  m.type.except <- list("gengneiting"                 )
...
#      }
  ...
  simulate <- function(cp, param=c("vario", "field", "simu", "rs")) {
    ...
    if (any("vario" == param) || !is.null(link.fct)) { 
      ...
      M <- PrepareModel(cp$model, trend=cp$mean)$model
      err <- !is.null(try(vdim <- .Call("CheckModelUser", M,
                                        as.integer(dim), as.integer(dim),
                                        TRUE, PACKAGE="RandomFields")))

      covvalue <- if (err) rep(NA, length(covx)) else 
      get(c("CovarianceFct","Variogram")[cp$variogram + 1])(x=covxx,
                                                            model=M,
                                                            param=NULL,
                                                            dim=dim)
      link.cov <- NULL
      ...
      if (!is.na(covvalue[1]) && !is.null(link.fct)) {
        if (cp$variogram) {
          corvalue <- CovarianceFct(covxx, model=M, dim=dim)
          if (is.na(corvalue[1])) {
            linkname <- "* trafo"
            corvalue <- Variogram(covxx, model=M, dim=dim)
      ...
      M <- PrepareModel(cp$model, trend=cp$mean)$model
      err <- !is.null(try(vdim <- .Call("CheckModelUser", M,
                                        as.integer(dim), as.integer(dim), TRUE,
    ...
    if (any(param %in% c("simu", "field"))) {
      if ("simu" %in% param) {
        ...
        z <- if (err) {
          if (is.null(y)) z <- rep(NA, length(x))
          else z <- array(NA, dim=c(length(x), length(y)))
        } else GaussRF(x, y, model=M, param=NULL, trend=cp$mean,
          ...
          switch(pmatch(link.fct, linkfctlist),
                 {
                   ...
                   z <- MaxStableRF(x, y, model=M, maxstable='extremalGauss',
                                    grid=TRUE, register=register)
                 },
                 stop("unknown link function")
                 )
  ...
  covlist <- c("bessel","cauchy","cauchytbm","circular",
               "cone","cubic","dampedcosine",
               "DeWijsian","CDeWijsian","exponential",
               "FD", "fractalB", "fractgauss", "gauss",
               "genB", "gencauchy", # "gengneiting",
               "gneiting",      
               "hyperbolic", "lgd1", "Matern", "nugget",
               "penta",# "power",
  ...
  exprlist <- c(expression(2^a *Gamma(a+1)*x^{-a}* J[a](x)),
                ...
                expression(log(1.0 + x^alpha)), # dewij
                expression((1.0 - log(1.0 + x^alpha) / log(1.0 +r^alpha))*(x<r)),
                expression(e^{-x}),
                # "FD",...
               ...
               # expression(see.manual),
                ...
                expression({1- b * (a+b)^{-1} * x^a} * (x <= 1) +
                    a * (a+b)^{-1} *x ^b  * (x>1)),
                expression(2^{1-nu}* Gamma(nu)^{-1}* (sqrt(2 * nu) * x)^nu *
                    K[nu](sqrt(2 * nu) * x)),
                expression((x==0)),
                # "penta",...
                ...
                expression(sin(x)/x),
                # "whittlematern",...
                expression(2^{1-nu}* Gamma(nu)^{-1}* x^nu * K[nu](x))
                )
  if (dim == 2) {
    dim2select <- -c(3, 7, 9, 11, 13)
    covlist <- covlist[dim2select]
    exprlist <- exprlist[dim2select]
  }
    
  expr <- rep(expression("C(x) unknown"), n)
  idx <- pmatch(covlist, namen)
  if (any(is.na(idx))) {
    stop("programming error -- inform maintainer")
  }
  ...
  if (is.null(model$model[[1]])) {
    ...
    cur.m[[covnr]] <-
      list(model=list(list(model=namen[covnr], var=all.param[2],
             kappas=npar[[covnr]]),
             "+",
             list(model="nugget", var=all.param[2])))  
    if (anisotropy) {
      cur.m[[covnr]]$model[[1]]$aniso <- diag(1/scale[covnr], 2)
      cur.m[[covnr]]$model[[3]]$aniso <- diag(2)
    } else {
      cur.m[[covnr]]$model[[1]]$scale <- scale[covnr]
      cur.m[[covnr]]$model[[3]]$scale <- 1
    }
  } else {
    cur.m[[covnr]] <- model

    if (anisotropy) {
      SVD <- svd(cur.m[[covnr]]$model[[1]]$aniso)
      u <- SVD$u[,1]
      if (u[1] < 0) u <- -u
      cur.m[[covnr]]$diag <- 1 / SVD$d
      if (is.na(varioangle)) { # no empirical variogram with 2 directions 
        cur.m[[covnr]]$angle <- acos(u[1]) / pi * 180
        if (u[2]<0) cur.m[[covnr]]$angle <- 180 - cur.m[[covnr]]$angle
      } else  cur.m[[covnr]]$angle <- (varioangle + 360) %% 180
    }
  } # else, is.null(model$model[[1]] 

  
  cur.m[[covnr]]$varioangle <- NA  ## default: varioangle follows angle
  cur.m[[covnr]]$PracticalRange <- RFparameters()$PracticalRange
  cur.m[[covnr]]$variogram <-  variogram
  cur.m[[covnr]]$mean <- Mean
  cur.m[[covnr]]$link <- FALSE
  oldnr <- covnr
  ...
  repeat {
    ...
    if (is.null(cur.m[[covnr]])) {
      cur.m[[covnr]] <- cur.m[[oldnr]] ## var, nugget, (scale/aniso) mean
      cur.m[[covnr]]$model[[1]]$model <- namen[covnr]     
      ## event fall unterscheidung npar[[covr]] == / != 0
      cur.m[[covnr]]$model[[1]]$kappas <- npar[[covnr]]
      if (!cur.m[[covnr]]$PracticalRange) {
        if (anisotropy) {
          cur.m[[covnr]]$model[[1]]$aniso <- diag(1/scale[covnr], 2)
          cur.m[[covnr]]$angle <- 0
          cur.m[[covnr]]$diag <- rep(scale[covnr], 2)
        }
        else cur.m[[covnr]]$model[[1]]$scale <- scale[covnr]
      }
    } else {
      cur.m[[covnr]]$PracticalRange <- cur.m[[oldnr]]$PracticalRange
      cur.m[[covnr]]$variogram <- cur.m[[oldnr]]$variogram
      cur.m[[covnr]]$mean <- cur.m[[oldnr]]$mean
    }

    ## define menue points that depend on the choice of the model
    kappa.entry <- NULL
    if ((l <- length(cur.m[[covnr]]$model[[1]]$kappas)) > 0) { 
      for (i in 1:l) {
    ...
    entry <- c(list(
                    if (!update) list(name="simulate", val="simulate",
                                      param=c("simu", "rs")),
                    list(name=cur.m[[covnr]]$model[[1]]$model, var=NULL,
...
#    str(cur.m)
#    str(cur.m[[covnr]]$model)
#    str( PrepareModel(cur.m[[covnr]]$model))
     options(warn=-1)
#   print(try(vdim <- .Call("CheckModelUser",
#                          PrepareModel(cur.m[[covnr]]$model)$model,
#                    as.integer(dim), as.integer(dim), TRUE,
#                       PACKAGE="RandomFields"))
#                )
    
    if(!is.null(try(vdim <- .Call("CheckModelUser",
                                  PrepareModel(cur.m[[covnr]]$model)$model,
                                  as.integer(dim), as.integer(dim), TRUE,
                                  PACKAGE="RandomFields"))
                )) {
     ...
     simulate(cp=cur.m[[covnr]])

##     cur.m[[covnr]] <- ## menu call
##       eval.parameters("cp", entry,
##                        update=update,  simulate=simulate,
##                        dev = par.dev, cp=cur.m[[covnr]],
...
##                        )
  ...
  if (anisotropy && !update) {
    f <- cur.m[[oldnr]]$angle * pi / 180
    u <- matrix(c(cos(f), sin(f), -sin(f), cos(f)), ncol=2)
    cur.m[[oldnr]]$model[[1]]$aniso <- u %*% (1/cur.m[[oldnr]]$diag * t(u))
  }
  return(PrepareModel(cur.m[[oldnr]]$model))
}
