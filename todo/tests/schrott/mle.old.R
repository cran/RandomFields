...
######################################################################
  ...
  LStarget <- function(variab) {
    ...
    model.values <-
     .C("VariogramNatSc", bin.centers, bins, covnr, as.double(param),
        as.integer(pm$genuinek), 
...
###########################      transform     #######################
  ...
  if (is.list(model) && is.list(unlist(model))) {
    ...
    model.nan <-
      convert.to.readable(PrepareModel(model.nan,NULL ), allow="list")
...
############################    model       ##########################
  ...
  data <- as.vector(data)
  pm <- PrepareModel(model, param, trend=trend)
  ...
  if (!is.null(ctr$param))
    ...
    pm <- PrepareModel(ctr$model, ctr$param, trend=trend)
...
##############      find upper and lower bounds      #################  
  ...
  if (is.list(lower)) {
    ...
    bndtst <- PrepareModel(lower, trend=trend)
    ...
    bndtst <- PrepareModel(upper, trend=trend)
    bndtst$param <- rep(NA, length(bndtst$param))
    if (test!=Unlist(convert.to.readable(bndtst, allowed="list")))
      stop(structuremismatch)
    ...
    if (is.vector(param)) {
      ...
      lower <- convert.to.readable(PrepareModel(model, lower, trend=trend),
                                   allowed="list")
      upper <- convert.to.readable(PrepareModel(model, upper, trend=trend),
                                   allowed="list")
      ...
      upper <- convert.to.readable(PrepareModel(model,
               matrix(upper, ncol=ncol(param), nrow=nrow(param)), trend= trend),
                                   allowed="list")
...
### now the bounds and starting values are set...
  ...
  lower <- PrepareModel(model=lower, trend=trend,
                        named=PrintLevel>1)$par
  upper <- PrepareModel(model=upper,  trend=trend,
                        named=PrintLevel>1)$par
  lower[is.nan(PARAM)] <- -Inf ## nan should not be checked -- this is
  ##                              completely up to the user ...
  upper[is.nan(PARAM)] <- Inf
  
  start <- PrepareModel(model=start, trend=trend)$par
  parscale <- 
    PrepareModel(model=parscale,  trend=trend)$par
  scale.pos <-
    PrepareModel(model=scale.pos,  trend=trend)$par
